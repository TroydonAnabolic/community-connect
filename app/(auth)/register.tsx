...
const ROLES: { key: UserRole; label: string; description: string; icon: string }[] = [
  { key: 'senior', label: 'Senior', description: 'I am a community member (60+)', icon: '🧓' },
  { key: 'caregiver', label: 'Caregiver', description: 'I support seniors in my life', icon: '🤲' },
  { key: 'organisation', label: 'Organisation', description: 'We run community programmes', icon: '🏢' },
];
...
export default function RegisterScreen() {
  ...
  async function handleRegister() {
    ...
    try {
      setLoading(true);
      await registerUser(email.trim(), password, displayName.trim(), selectedRole!);
        ...
        <ScrollView contentContainerStyle={styles.container} keyboardShouldPersistTaps="handled">
          ...
          {step === 1 ? (
            <>
              ...
              <Text style={styles.subheading}>Choose the option that best describes you</Text>
                ...
                {ROLES.map((role) => (
                  ...
                  >
                    ...
                    <View style={styles.roleText}>
                      <Text style={[styles.roleLabel, selectedRole === role.key && styles.roleLabelSelected]}>
                        {role.label}
                      </Text>
                      <Text style={styles.roleDesc}>{role.description}</Text>
                    </View>
                    <View style={[styles.radioOuter, selectedRole === role.key && styles.radioOuterSelected]}>
                      {selectedRole === role.key && <View style={styles.radioInner} />}
                    </View>
                  </TouchableOpacity>
                ))}
              </View>
              <TouchableOpacity style={styles.btnPrimary} onPress={handleNextStep} accessibilityRole="button">
...
const styles = StyleSheet.create({
  ...
  heading: { fontSize: 28, fontWeight: '700', color: Colors.textPrimary, marginBottom: Spacing.sm },
  subheading: { fontSize: FontSizes.md, color: Colors.textSecondary, marginBottom: Spacing.xl, lineHeight: 22 },
  ...
  roleText: { flex: 1 },
  roleLabel: { fontSize: FontSizes.lg, fontWeight: '600', color: Colors.textPrimary },
  roleLabelSelected: { color: Colors.primaryDark },
  roleDesc: { fontSize: FontSizes.sm, color: Colors.textSecondary, marginTop: 2 },
  radioOuter: {
