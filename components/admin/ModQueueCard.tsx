...
export default function ModQueueCard({ item, onApprove, onWarn, onRemove }: Props) {
  const tc = TYPE_COLORS[item.contentType] ?? { bg: Colors.surfaceSecondary, text: Colors.textSecondary };
    ...
    <View style={styles.card}>
      ...
      <View style={styles.header}>
        <View style={[styles.typeBadge, { backgroundColor: tc.bg }]}>
          <Text style={[styles.typeText, { color: tc.text }]}>{item.contentType}</Text>
        </View>
      ...
      {item.contentPreview ? (
        <View style={styles.preview}>
          <Text style={styles.previewText} numberOfLines={4}>{item.contentPreview}</Text>
        </View>
      ) : null}
      ...
      <View style={styles.actions}>
        ...
        >
          ...
          <Text style={[styles.actionText, { color: Colors.warning }]}>Warn</Text>
        </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  typeBadge: { paddingHorizontal: 10, paddingVertical: 3, borderRadius: Radii.full },
  typeText: { fontSize: FontSizes.xs, fontWeight: '700', textTransform: 'capitalize' },
  time: { fontSize: FontSizes.xs, color: Colors.textTertiary },
  reasonRow: { flexDirection: 'row', alignItems: 'center', gap: 5, marginBottom: Spacing.sm },
  reason: { fontSize: FontSizes.sm, color: Colors.warning, fontStyle: 'italic', flex: 1 },
  preview: {
    backgroundColor: Colors.background, borderRadius: Radii.md,
    padding: Spacing.md, marginBottom: Spacing.md,
  },
  previewText: { fontSize: FontSizes.md, color: Colors.textPrimary, lineHeight: 22 },
  actions: { flexDirection: 'row', gap: Spacing.sm },
  actionBtn: {
    flex: 1, flexDirection: 'row', alignItems: 'center', justifyContent: 'center',
    gap: 5, paddingVertical: 9, borderRadius: Radii.md, borderWidth: 1.5,
  },
  approveBtn: { borderColor: Colors.success, backgroundColor: Colors.successLight },
  warnBtn:    { borderColor: Colors.warning, backgroundColor: Colors.warningLight },
